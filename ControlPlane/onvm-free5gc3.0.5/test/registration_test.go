...
*/
	...
	_, err = conn.Write(sendMsg)
	assert.Nil(t, err)
    fmt.Println("Finish PDU Session : ")

	time.Sleep(100 * time.Millisecond)
    cmd := exec.Command("bash", "-c", "echo -n 'start sending data' | nc -u -w1 192.168.24.3 7777")
    _, err = cmd.Output()
    if err != nil {
        fmt.Println(err)
        assert.Nil(t, err)
    ...
    fmt.Println("Starting handover")
    cmd = exec.Command("bash", "-c", "echo -n 'handover init' | nc -u -w1 192.168.24.3 7777")
    _, err = cmd.Output()
    if err != nil {
        fmt.Println(err)
        assert.Nil(t, err)
	...
	sendMsg, err = test.GetInitialContextSetupResponseForServiceRequest(targetUe.AmfUeNgapId, targetUe.RanUeNgapId, "10.100.200.1")
	assert.Nil(t, err)
	_, err = conn2.Write(sendMsg)
	assert.Nil(t, err)
    ...
    cmd = exec.Command("bash", "-c", "echo -n 'stop' | nc -u -w1 192.168.24.3 7777")
    _, err = cmd.Output()
    if err != nil {
        fmt.Println(err)
        assert.Nil(t, err)
